...
import psutil
import config
from ISTKHAR_CHATBOT import _boot_
from ISTKHAR_CHATBOT import get_readable_time
from ISTKHAR_CHATBOT import ISTKHAR_CHATBOT, mongo
...
from ISTKHAR_CHATBOT.database.chats import get_served_chats, add_served_chat
from ISTKHAR_CHATBOT.database.users import get_served_users, add_served_user
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup, Message, CallbackQuery
from ISTKHAR_CHATBOT.modules.helpers import (
    START,
    START_BOT,
    PNG_BTN,
    CLOSE_BTN,
...
BOT = "https://files.catbox.moe/bem47n.jpg"
IMG = [
    "https://files.catbox.moe/bem47n.jpg",
    "https://files.catbox.moe/bem47n.jpg",
    "https://files.catbox.moe/v0qvvi.jpg",
    "https://files.catbox.moe/v0qvvi.jpg",
    "https://files.catbox.moe/v0qvvi.jpg",
    "https://files.catbox.moe/v0qvvi.jpg",
    "https://files.catbox.moe/v0qvvi.jpg",
    "https://files.catbox.moe/f8i9s1.jpg",
    "https://files.catbox.moe/f8i9s1.jpg",
    "https://files.catbox.moe/f8i9s1.jpg",
    "https://files.catbox.moe/f8i9s1.jpg",
    "https://files.catbox.moe/f8i9s1.jpg",
...
from ISTKHAR_CHATBOT import db
...
@ISTKHAR_CHATBOT.on_message(filters.new_chat_members)
async def welcomejej(client, message: Message):
    ...
    try:
        for member in message.new_chat_members:
            ...
            if member.id == ISTKHAR_CHATBOT.id:
                try:
                    ...
                    pass
                try:
                    invitelink = await ISTKHAR_CHATBOT.export_chat_invite_link(message.chat.id)
                ...
                try:
                    groups_photo = await ISTKHAR_CHATBOT.download_media(
                ...
                count = await ISTKHAR_CHATBOT.get_chat_members_count(chat.id)
                chats = len(await get_served_chats())
                ...
                try:
                    ...
                    if OWNER:
                        await ISTKHAR_CHATBOT.send_photo(
                            int(OWNER_ID),
                            photo=chat_photo,
                            caption=msg,
...
@ISTKHAR_CHATBOT.on_cmd(["start", "aistart"])
async def start(_, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        if m.chat.photo:
            try:
                userss_photo = await ISTKHAR_CHATBOT.download_media(m.chat.photo.big_file_id)
                await umm.delete()
                if userss_photo:
        ...
        await m.reply_photo(photo=chat_photo, caption=START.format(ISTKHAR_CHATBOT.mention or "can't mention", users, chats, UP), reply_markup=InlineKeyboardMarkup(START_BOT))
        await add_served_user(m.chat.id)
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(f"{m.chat.first_name}", user_id=m.chat.id)]])
        await ISTKHAR_CHATBOT.send_photo(int(OWNER_ID), photo=chat_photo, caption=f"{m.from_user.mention} ʜᴀs sᴛᴀʀᴛᴇᴅ ʙᴏᴛ. \n\n**ɴᴀᴍᴇ :** {m.chat.first_name}\n**ᴜsᴇʀɴᴀᴍᴇ :** @{m.chat.username}\n**ɪᴅ :** {m.chat.id}\n\n**ᴛᴏᴛᴀʟ ᴜsᴇʀs :** {users}", reply_markup=keyboard)
...
@ISTKHAR_CHATBOT.on_cmd("help")
async def help(client: ISTKHAR_CHATBOT, m: Message):
    if m.chat.type == ChatType.PRIVATE:
...
async def ping(_, message: Message):
    ...
    await loda.edit_text(
        text=f"нey вαву!!\n{ISTKHAR_CHATBOT.name} ᴄʜᴀᴛʙᴏᴛ ιѕ alιve  αnd worĸιng ғιne wιтн a pιng oғ\n\n**➥** `{ms}` ms\n**➲ ᴄᴘᴜ:** {CPU}\n**➲ ʀᴀᴍ:** {RAM}\n**➲ ᴅɪsᴋ:** {DISK}\n**➲ ᴜᴘᴛɪᴍᴇ »** {UP}\n\n<b>||** ⋆ʟᴏᴠᴇ ᴡɪᴛʜ⋆ [ XQUEEN ](https://t.me/{OWNER_USERNAME}) **||</b>",
...
@ISTKHAR_CHATBOT.on_message(filters.command("stats"))
...
from ISTKHAR_CHATBOT import ISTKHAR_CHATBOT


@ISTKHAR_CHATBOT.on_cmd("id")
                ...
                for chat in chats:
                    ...
                    try:
                        if broadcast_type == "reply":
                            m = await ISTKHAR_CHATBOT.forward_messages(
                                chat_id, message.chat.id, [broadcast_content.id]
                            )
                        else:
                            m = await ISTKHAR_CHATBOT.send_message(
